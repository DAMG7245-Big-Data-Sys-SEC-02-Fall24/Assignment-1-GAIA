...
resource "null_resource" "schema_setup" {
  ...
  provisioner "local-exec" {
    command = <<EOT
      PGPASSWORD=${random_password.db_password.result} psql -h ${google_sql_database_instance.postgres_instance.public_ip_address} -U admin -d assignment-1-postgres -f schema.sql
    EOT
  }
}
